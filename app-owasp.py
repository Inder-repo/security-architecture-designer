...
def _generate_custom_asvs_entries(custom_reqs_data):
    ...
    for scenario, req_list in custom_reqs_data.items():
        prefix = scenario.replace(" -> ", "_").replace(" ", "").replace("-", "")
        for i, req in enumerate(req_list):
            asvs_id = f"CUSTOM-{prefix}-{i+1}"
            # Assign a reasonable ASVS level and GRC mapping. L1 for general, L2/L3 for more advanced.
            # For simplicity, default to L1 and 'Custom' GRC.
            level = "L1"
            grc_mapping = "Custom"
            
            # You could add logic here to infer higher levels or specific GRC
            if "multi-factor" in req.lower() or "mTLS" in req.lower():
                level = "L2"
            elif "row/column-level security" in req.lower():
                level = "L3" # More advanced data access control
            
            # Simple GRC mapping suggestions
            if "authentication" in req.lower() or "mfa" in req.lower():
                grc_mapping = "NIST 800-63B IA-2"
            elif "access control" in req.lower() or "least privilege" in req.lower() or "rbac" in req.lower():
...
def _update_flow_mappings_with_custom_asvs(initial_flow_mappings, generated_custom_asvs):
    ...
    for custom_entry in generated_custom_asvs:
        ...
        scenario = custom_entry['ASVS_ID'].split('-')[1].lower() # e.g., 'USERAPP'

        if 'userapp' in scenario:
            if 'User Login' not in updated_mappings: updated_mappings['User Login'] = set()
            updated_mappings['User Login'].add(asvs_id)
            if 'HTTPS' not in updated_mappings: updated_mappings['HTTPS'] = set()
            updated_mappings['HTTPS'].add(asvs_id)
        elif 'appapp' in scenario:
            if 'API-to-API' not in updated_mappings: updated_mappings['API-to-API'] = set()
            updated_mappings['API-to-API'].add(asvs_id)
            if 'HTTPS' not in updated_mappings: updated_mappings['HTTPS'] = set() # Often API is over HTTPS
            updated_mappings['HTTPS'].add(asvs_id)
        elif 'appdata' in scenario:
            if 'Database Query' not in updated_mappings: updated_mappings['Database Query'] = set()
            updated_mappings['Database Query'].add(asvs_id)
            if 'File Transfer' not in updated_mappings: updated_mappings['File Transfer'] = set() # For data movement
            updated_mappings['File Transfer'].add(asvs_id)
        elif 'databaseadmin' in scenario: # Specific to Database Admin
            if 'Database Query' not in updated_mappings: updated_mappings['Database Query'] = set()
            updated_mappings['Database Query'].add(asvs_id)
            if 'Other' not in updated_mappings: updated_mappings['Other'] = set() # For general admin tasks
            updated_mappings['Other'].add(asvs_id)
        elif 'administratorapplication' in scenario or 'administratorplatform' in scenario:
            if 'User Login' not in updated_mappings: updated_mappings['User Login'] = set()
            updated_mappings['User Login'].add(asvs_id)
            if 'Other' not in updated_mappings: updated_mappings['Other'] = set() # General admin activities
            updated_mappings['Other'].add(asvs_id)
            if 'HTTPS' not in updated_mappings: updated_mappings['HTTPS'] = set()
            updated_mappings['HTTPS'].add(asvs_id)
